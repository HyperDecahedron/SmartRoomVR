...
user = 101
MAX_JITTERING = 250
PRESSURE_NORMALIZATION = 0.9 # <1 to increase the max, >1 to decrease the max
...
# UDP communication
UNITY_IP = "192.168.1.117"  # UPDATE THIS WITH THE QUEST IP
UNITY_PORT = 5052
...
def classification_loop():
    ...
    model_dir = f"C:/Quick_Disk/ProjectFrog_EMG/Pressure_Classifier/Notebooks/models"
    pressure_regressor = joblib.load(f"{model_dir}/Pressure_regressor_user_frog_{user}.joblib")  # whole pipeline already included
    ...
    while True:
        ...
        if len(buffer) >= window_size_samples: 
            ...
            message = f"{jittering},{pressure_values[0]},{pressure_values[1]},{pressure_values[2]}"
            try:
